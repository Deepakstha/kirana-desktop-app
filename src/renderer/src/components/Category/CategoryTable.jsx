...
export default function CategoryTable({ results }) {
  ...
  const handleDelete = async (cat_id, category_name) => {
    Swal.fire({
      text: `Are you sure to delete ${category_name} Category?`,
      icon: 'warning',
      width: 350,
      ...
      if (result.isConfirmed) {
        let resultsData = await window.electronic.invoke('deleteCategory', cat_id)
        if (resultsData == 'Category Deleted') {
          ...
          {(rowsPerPage > 0
            ...
            <TableRow key={row.cat_id}>
              ...
              <TableCell align="right">
                {' '}
                <Link to={`/update-category/${row.cat_id}`}>
                  <Button>Edit</Button>
                </Link>{' '}
                <Button onClick={() => handleDelete(row.cat_id, row.category_name)}>Delete</Button>{' '}
              </TableCell>
            </TableRow>
          ))}
          {emptyRows > 0 && (
